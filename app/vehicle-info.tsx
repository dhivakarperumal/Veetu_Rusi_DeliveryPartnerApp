        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          <View className="px-6 mt-6 pb-10">
            ...
            <InfoCard
              ...
              value={user?.license_holder_name}
              icon="user"
            />
            <InfoCard
              label="License Issue Date"
              value={user?.license_issue_date}
              icon="calendar"
            />
            <InfoCard
            ...
            <InfoCard
              ...
              value={user?.insurance_expiry_date}
              icon="alert-circle"
            />
            <InfoCard
              label="Pollution Certificate No."
              value={user?.pollution_certificate_number}
              icon="wind"
            />
            ...
            <InfoCard
              ...
              value={user?.preferred_distance}
              icon="map"
            />
            <InfoCard
              label="Delivery Radius"
              value={user?.delivery_radius}
              icon="navigation"
            />
            <InfoCard
              label="Driving Experience"
              value={user?.driving_experience}
              icon="award"
            />
            <InfoCard
              label="Available Areas"
              value={user?.available_areas}
              icon="map-pin"
            />
            <InfoCard
              label="Online Status"
              value={user?.online_status}
              icon="wifi"
            />
            <InfoCard
            ...
            <BoolCard
              label="Afternoon"
              value={user?.available_time_afternoon}
            />
...
function BoolCard({ label, value }: any) {
  ...
  return (
    <View className="bg-white rounded-2xl px-5 py-4 mb-3 border border-gray-100 shadow-sm flex-row justify-between items-center">
      ...
      <View
        className={`px-3 py-1 rounded-full ${active ? "bg-primary-lightGreen" : "bg-gray-100"}`}
      >
        <Text
          className={`text-xs font-bold ${active ? "text-primary-brandGreen" : "text-gray-400"}`}
        >
