...
const STATUS_STYLE: Record<string, { bg: string; text: string; border: string }> = {
  ...
  "Searching Delivery Partner": { bg: "#451a03", text: "#fde047",  border: "#92400e" },
  /* ── Delivery Partner statuses (from user's STATUS_STYLE) ── */
  "Delivery Partner Assigned":  { bg: "#083344", text: "#67e8f9",  border: "#155e75" },
...
const DELIVERY_STATUSES = [
  "Delivery Partner Assigned", "Picked Up", "Start Ride",
  "Reached Location", "Waiting for Customer", "Out for Delivery", "Delivered",
];
...
const IN_PROGRESS_STATUSES = [
  "Accepted", "Preparing", "Food Ready", "Packing",
  "Delivery Partner Assigned", "Picked Up", "Start Ride",
  "Reached Location", "Waiting for Customer", "Out for Delivery",
];
...
function UpdateStatusModal({
  order, visible, onClose, onSaved,
}: { order: any; visible: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <TouchableOpacity
        ...
        onPress={onClose}
      />
      <View className="bg-white rounded-t-3xl px-6 pt-6 pb-10" style={{ position: "absolute", bottom: 0, left: 0, right: 0 }}>
        {/* Handle */}
        ...
        <View className="flex-row justify-between items-start mb-5">
          ...
          <View>
            <Text className="text-gray-400 text-xs font-semibold text-right">Amount</Text>
            <Text className="text-black font-extrabold text-base text-right">{fmt(order?.total_amount)}</Text>
          </View>
        </View>

        <Text className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-3">Update Status</Text>
            ...
            return (
              <TouchableOpacity
                ...
                className={`flex-row items-center justify-between px-5 py-4 rounded-2xl border-2 mb-2 ${
                  isSelected ? "border-primary-darkGreen bg-primary-lightGreen" : "border-gray-100 bg-gray-50"
                }`}
              >
                <View className="flex-row items-center">
                  <Text className="text-xl mr-3">{STATUS_ICONS[s]}</Text>
                  <Text className={`font-bold text-sm ${isSelected ? "text-primary-darkGreen" : "text-gray-700"}`}>{s}</Text>
                </View>
                <View className="flex-row items-center gap-2">
                  {isCurrent && !isSelected && (
                    <View className="bg-gray-200 px-2 py-0.5 rounded-full mr-2">
                      <Text className="text-gray-500 text-[9px] font-black uppercase">Current</Text>
                    </View>
                  )}
                  <View className={`w-5 h-5 rounded-full border-2 items-center justify-center ${
                    isSelected ? "bg-primary-darkGreen border-primary-darkGreen" : "border-gray-300"
                  }`}>
        ...
        <TouchableOpacity
          ...
          style={{ opacity: (saving || !selectedStatus || selectedStatus === order?.status) ? 0.5 : 1 }}
        >
          {saving ? (
...
const FILTER_TABS = [
  { label: "All",                        status: "All",                        bg: "#1e293b", text: "#94a3b8", border: "#334155" },
  { label: "Assigned",                   status: "Delivery Partner Assigned",  bg: "#083344", text: "#67e8f9", border: "#155e75" },
  { label: "Picked Up",                  status: "Picked Up",                  bg: "#082f49", text: "#7dd3fc", border: "#075985" },
  { label: "Start Ride",                 status: "Start Ride",                 bg: "#172554", text: "#93c5fd", border: "#1e40af" },
  { label: "Reached",                    status: "Reached Location",           bg: "#4a044e", text: "#f0abfc", border: "#86198f" },
  { label: "Waiting",                    status: "Waiting for Customer",       bg: "#451a03", text: "#fcd34d", border: "#92400e" },
  { label: "Out for Delivery",           status: "Out for Delivery",           bg: "#1e1b4b", text: "#a5b4fc", border: "#3730a3" },
  { label: "Delivered",                  status: "Delivered",                  bg: "#022c22", text: "#6ee7b7", border: "#065f46" },
];
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-main" edges={['left', 'right', 'bottom']}>
      ...
      <ScrollView
        ...
        style={{ flexGrow: 0 }}
        contentContainerStyle={{ paddingHorizontal: 24, paddingVertical: 4, alignItems: 'center' }}
        className="mt-3 mb-3"
      >
        {FILTER_TABS.map((tab) => {
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-gray-400 mt-4 text-sm font-semibold text-center">{error}</Text>
          <TouchableOpacity onPress={fetchOrders} className="mt-5 bg-primary-darkGreen px-6 py-3 rounded-xl">
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={Colors.primary.darkGreen} />}
        >
            ...
            {filteredOrders.length > 0 ? (
              ...
              <View className="items-center mt-16">
                ...
                <Text className="text-gray-400 mt-4 font-semibold text-sm">
                  No orders{activeStatus !== "All" ? ` for "${activeStatus}"` : ""}.
                </Text>
                <Text className="text-gray-300 mt-1 text-xs">Pull down to refresh</Text>
              </View>
            )}
          </View>
        </ScrollView>
...
function OrderCard({ order, onEdit }: { order: any; onEdit: () => void }) {
  ...
  const address   = [order.street_address, order.city, order.district, order.state]
    .filter(Boolean).join(", ") || order.delivery_address || "—";
  const payment   = order.payment_method || "COD";

  const canUpdate = DELIVERY_STATUSES.includes(status) && status !== "Delivered" && status !== "Cancelled";
  ...
  return (
    <View className="bg-white rounded-3xl shadow-sm border border-gray-50 mb-4 overflow-hidden">
      ...
      <View className="flex-row justify-between items-center px-5 pt-5 pb-4 border-b border-gray-50">
        ...
        >
          <Text style={{ color: style.text }} className="font-extrabold text-[9px] uppercase tracking-widest">
            {status}
          </Text>
        </View>
      </View>
      ...
      <View className="px-5 py-4 border-b border-gray-50">
        <View className="flex-row items-center">
          ...
          <View>
            ...
            {phone ? <Text className="text-gray-400 text-xs mt-0.5">{phone}</Text> : null}
          </View>
        </View>

        {address !== "—" && (
          <View className="flex-row items-start mt-3 bg-gray-50 rounded-2xl px-4 py-3">
            <Feather name="map-pin" size={13} color={Colors.primary.darkGreen} style={{ marginTop: 2 }} />
            <Text className="text-gray-600 text-xs ml-2 flex-1 leading-5">{address}</Text>
          </View>
        )}
      </View>
